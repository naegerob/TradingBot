...
# X-axis values
x_values = list(range(len(original.text)))
x_valuesSma = list(range(len(sma.text)))
x_valuesRsi = list(range(len(rsi.text)))
windowSma = 20
windowRsi = 14
rsiLow = 30
rsiHigh = 70
...
original = original.text.replace("[", "").replace("]", "").replace("'", "")
sma = sma.text.replace("[", "").replace("]", "").replace("'", "")
sma = sma.split(",")
